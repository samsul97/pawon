angular.module('App').controller('LoginController', function ($rootScope, $scope, $http, $mdToast, $route, $timeout, request) {
	...
	if (root.isCookieExist()) {
		root.isLogin = false;
		window.location.href = '#dashboard_pengunjung';
		$mdToast.show($mdToast.simple().content('Login Berhasil').position('bottom right'));
	...
	$rootScope.pagetitle = 'Masuk Akun Pawon Anda';
	...
	self.doLogin = function () {
		...
		request.login(self.userdata).then(function (result) {
			...
			$timeout(function () { // give delay for good UI
				self.submit_loading = false;
				if (resp == "") {
				    $mdToast.show($mdToast.simple().content('Login Gagal').position('bottom right'));
				    return;
				}
